...
export default function () {
    ...
    const requests = Array.from({length: 2}, () => {
        ...
        return {
            ...
            params: {
                headers,
                tags: {input_summary: input.slice(0, 20)},
            },
            input,
        };
    });
    ...
    for (let i = 0; i < responses.length; i++) {
        ...
        const input = requests[i].input;

        let hasForbiddenTerm = null;

        try {
            if (res.body) {
                const parsed = JSON.parse(res.body);
                if (parsed?.data?.hasForbiddenTerm !== undefined) {
                    hasForbiddenTerm = parsed.data.hasForbiddenTerm;
                }
            } else {
                console.warn(`Empty response body for input: "${input.slice(0, 20)}"`);
            }
        } catch (e) {
            console.warn(`JSON parse failed for input: "${input.slice(0, 20)}"\nBody: ${res.body}`);
        }

        check(res, {
            'status is 200': (r) => r.status === 200,
            'body has hasForbiddenTerm': () => typeof hasForbiddenTerm === 'boolean',
        });
    }
}
// k6 run  --out influxdb=http://localhost:8086/k6 ./scripts/api_test_memory.js
